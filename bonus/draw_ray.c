...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/28 17:41:59 by pc                #+#    #+#             */
/*   Updated: 2024/10/28 17:41:59 by pc               ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "includes/cub3d.h"

int	is_wall_ray(t_mlx *mlx, double x, double y, int rays)
{
	int	map_x;
	int	map_y;
	...
	if (!mlx->ray.door[rays])
		mlx->ray.door[rays] = !mlx->ray.door[rays];
	if (map_x < mlx->map.size_x || map_x > 0 || map_y > 0 \
		|| map_y < mlx->map.size_y)
	{
		if (mlx->map.matrix[map_y][map_x] == 'D')
		{
			mlx->ray.door[rays] = !mlx->ray.door[rays];
			return (1);
		}
		if (mlx->map.matrix[map_y][map_x] == '1')
			...
			return (0);
		else if (mlx->map.matrix[map_y][map_x] == 'O')
			return (0);
	}
	return (1);
}
...
int	perform_dda(t_mlx *mlx, int rays)
{
	...
	while (!hit)
	{
		...
		if (is_wall_ray(mlx, mlx->ray.ray_x[rays], mlx->ray.ray_y[rays], rays))
			hit = 1;
	}
	return (side);
}
